...
qtchartpluginFile="qtcharts-everywhere-"$openSource"src-$concVersion"
qtchartpluginDir="qtcharts-everywhere-src-$concVersion"

qtchartpluginXZ="$qtchartpluginFile.tar.xz"
qtchartplugin="https://download.qt.io/archive/qt/$mainVersion/$concVersion/submodules/$qtchartpluginXZ"

rm -rf qtchartpluginFile
if [ -f "$HOME/$qtchartpluginXZ" ]; then
    echo "$qtchartpluginXZ exists."
else 
    echo "$qtchartpluginXZ does not exist."
	wget $qtchartplugin
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "Desired $qtchartplugin is not found"
    exit 1
fi
...
if [ -d "$DIR" ]; then
  ...
  echo "qtbase is exist ${DIR}..."
	...
	tar xf $qtbaseXZ
	cd $HOME/$qtbaseDir
	./configure -opengl desktop -xcb -xcb-xlib -opensource -confirm-license -force-debug-info -prefix $INSTALLDIR -nomake examples -nomake tests -v |& tee -a $OUTPUT_FILE
	make -j8
	make install
	export QT_SELECT=5
	cd ..
...
tar xf $HOME/$qtchartpluginXZ
cd $HOME/$qtchartpluginDir
$INSTALLDIR/bin/qmake
make -j8
make install
cd ..
...
if [ -f "$HOME/$qtchartpluginXZ" ]; then
    rm -rf $HOME/$qtchartpluginXZ
    rm -rf $HOME/$qtchartpluginDir
fi
