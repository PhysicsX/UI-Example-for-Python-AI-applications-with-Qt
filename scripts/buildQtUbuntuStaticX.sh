...
INSTALLDIR="/usr/local/StaticQt$mainVersion$subVersion"
...
if [ -d "$DIR" ]; then
  ...
  echo "qtbase is exist ${DIR}..."
	...
	tar xf $qtbaseXZ
	cd $HOME/$qtbaseDir
	./configure -static -opengl desktop -xcb -xcb-xlib -opensource -confirm-license -force-debug-info -prefix $INSTALLDIR -nomake examples -nomake tests -v |& tee -a $OUTPUT_FILE
	make -j8
	make install
	export QT_SELECT=5
	cd ..
